...
`release/qwen38next`, PR vllm-project/vllm#53896 — still open, not in main)
...
**complete** embedding row from disk on every rank, bypassing the sharded
lookup *and its all-reduce entirely*. Since the stock path's contract is
...
- 2 KV heads × 256 head_dim: full-model KV ≈ 12 × 2 × 256 × 2 (K+V) × 2 B
  ≈ **24 KiB/token**; per rank at TP4 (1 replicated head) ≈ **12 KiB/token**.
- A 1M-token request ≈ ~15 GiB KV per rank (bf16). This is why the repo
  exists: one box can't hold that next to ~76 GiB of weights; a TP4 rank
  with ~20 GiB of weights can.
- Not in this ledger: GDN/mamba state cache, QSA indexer pools, raw-key
  ring, CUDA graphs — budget for tens of GiB before trusting any pool
...
image supersedes the day-0 tag, expect to re-path patches 1–6 and re-check
